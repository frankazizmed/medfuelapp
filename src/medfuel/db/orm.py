...
class JobRow(Base):
    ...
    requested_pages: Mapped[int] = mapped_column(Integer, default=4)
...
class ReportRunRow(Base):
    ...
    company_id: Mapped[str] = mapped_column(ForeignKey("companies.company_id"), index=True)
    job_id: Mapped[str | None] = mapped_column(ForeignKey("jobs.job_id"), index=True, nullable=True)
    pages_requested: Mapped[int] = mapped_column(Integer, default=4)
    pages_rendered: Mapped[int] = mapped_column(Integer, default=4)
